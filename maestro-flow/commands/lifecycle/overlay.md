...
After confirming the injection point, ask whether this overlay should chain to another skill upon completion. This enables the overlay's injected content to hand off to a skill via AskUserQuestion at runtime — similar to how `/maestro` chains commands via `Skill({ skill: "maestro-flow", args: "--cmd ... ..." })`.
...
On user selection:
- Proceed → Skill({ skill: "maestro-flow", args: "--cmd quality-review {phase}" })
- Alternative → Skill({ skill: "maestro-flow", args: "--cmd maestro-verify {phase}" })
- Skip → continue normally
```

Handoff rules:
- Always include a **"Skip"** option — the user can always decline the chain
- Use `Skill({ skill: "maestro-flow", args: "--cmd <name> ..." })` syntax consistent with maestro.md chainMap
